...
typedef struct{
	...
	size_t* thread_to_print;
	}thread_data_t;

void* run(void* data)
{
	thread_data_t* info = (thread_data_t*)data;
	while (*info->thread_to_print < info->thread_id)
     {
     } 
	printf("Hello from thread %zu of %zu\n", info->thread_id + 1, info->total_threads);
	++(*info->thread_to_print);
...
{	
	...
	for(size_t index = 0; index < thread_count; ++index){
		...
		thread_data[index].thread_to_print = &next_thread;
