...
int main(int argc, char *argv[])
{
	...
	else
	{
		if(my_rank == 0)
		{
			...
			{
				MPI_Send(&start, 1, MPI_UNSIGNED_LONG_LONG, target, 0, MPI_COMM_WORLD);
				MPI_Send(&end, 1, MPI_UNSIGNED_LONG_LONG, target, 0, MPI_COMM_WORLD);
			}
		}
		else
		{
			MPI_Recv(&start, 1, MPI_UNSIGNED_LONG_LONG, /*source*/ 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Recv(&end, 1, MPI_UNSIGNED_LONG_LONG, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
	}
