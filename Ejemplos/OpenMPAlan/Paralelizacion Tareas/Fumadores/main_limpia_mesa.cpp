...
int main(int argc, char* argv[])
{
	...
	bool materialesProvistos[3] = { false, false, false };	// arreglo de materiales provistos por h0 en cada iteración
		...
		while (it < ci)
		{
			# pragma omp master
			{
				...
				if (cA < probabilidades[0]) { // si h0 debe aprovisionar algún material
					...
					if (cM < probabilidades[1]) {  // debe aprovisionar TP
						materialesProvistos[0] = true;
						materialesProvistos[1] = true;
						materialesProvistos[2] = false;
					}
					else if (cM < probabilidades[2]) { // debe aprovisionar PC
						materialesProvistos[0] = false;
						materialesProvistos[1] = true;
						materialesProvistos[2] = true;
					}
					else { // debe aprovisionar TC
						materialesProvistos[0] = true;
						materialesProvistos[1] = false;
						materialesProvistos[2] = true;
					}
				}
			}
			...
			if (mid == 1) {	// comportamiento del h1 que tiene tabaco
				if (stdo_fumadores[0] == esperando)
					if (materialesProvistos[1] && materialesProvistos[2])
					{
			...
			if (mid == 2) {	// comportamiento del h2 que tiene papel
				if (stdo_fumadores[1] == esperando)
					if (materialesProvistos[0] && materialesProvistos[2])
					{
			...
			if (mid == 3) {	// comportamiento del h3 que tiene cerillas
				if (stdo_fumadores[2] == esperando)
					if (materialesProvistos[0] && materialesProvistos[1])
					{
			...
			# pragma omp master
			{
				...
				if (materialesProvistos[0] || materialesProvistos[1] || materialesProvistos[2])
				{
					materialesProvistos[0] = false;
					materialesProvistos[1] = false;
					materialesProvistos[2] = false;
					cout << "	Mesa limpiada" << endl;
				}
			}
