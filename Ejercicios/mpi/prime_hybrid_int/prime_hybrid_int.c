...
int main(int argc, char* argv[])
{
	...
	#pragma omp parallel num_threads(threads) default(none) shared(my_rank, process_count, hostname, my_start, my_finish, prime_count)
	{		
		#pragma omp for
	...
	MPI_Reduce(&threads, &all_threads, 1, MPI_INT, MPI_SUM,/*root*/ 0, MPI_COMM_WORLD);
	...
	if(my_rank == 0)
		fprintf(stdout, "%ld primes found in range [%zu, %zu[ in %.8lfs with %d processes and %d threads\n"
					,all_primes, start, end, seconds, process_count, all_threads);
	MPI_Finalize();
	return 0;
}
