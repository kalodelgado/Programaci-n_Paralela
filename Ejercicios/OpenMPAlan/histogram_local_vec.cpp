...
int main(int argc, char* argv[]) {
	...
	#pragma omp parallel num_threads( omp_get_num_procs() ) default(none) shared(data_count, data, bin_maxes, bin_count, min_meas, bin_counts) private(bin)
	{
		vector<int> bin_local_counts;
		bin_local_counts.resize(bin_count);
		#pragma omp for
		...
		{
			#pragma omp atomic
			bin_counts[i] += bin_local_counts[i];
		}
	}	
...
void gen_data(
	...
	srand(0);
	#pragma omp parallel for default(none) shared(min_meas, max_meas, data_count, data)
	for (int i = 0; i < data_count; i++)
